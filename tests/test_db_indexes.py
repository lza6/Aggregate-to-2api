...
class TestDBIndexes:
    ...
    def test_created_status_index_exists(self, tmp_db):
    ...
    def test_existing_indexes_still_present(self, tmp_db):
        """原有索引仍存在。"""
        _, conn, lock = tmp_db._get_write_conn()
        with lock:
...
class TestDayMonthColumn:
    ...
    def test_create_request_writes_day_month(self, tmp_db):
        """create_request 写入 day 和 month 列。"""
        tmp_db.create_request("test-day-month", "prompt", "1:1", False)
        tmp_db.flush()
        _, conn, lock = tmp_db._get_write_conn()
        with lock:
            row = conn.execute(
    ...
    def test_day_month_format(self, tmp_db):
        """day 格式 YYYY-MM-DD，month 格式 YYYY-MM。"""
        tmp_db.create_request("test-format", "prompt", "1:1", False)
        tmp_db.flush()
        _, conn, lock = tmp_db._get_write_conn()
        with lock:
            row = conn.execute(
...
class TestStatsWithDayMonth:
    ...
    def test_stats_daily_uses_day_column(self, tmp_db):
        """stats_daily 使用 day 列查询。"""
        tmp_db.create_request("s1", "p1", "1:1", False)
        tmp_db.flush()
        result = tmp_db.stats_daily(7)
        assert len(result) >= 1
        assert "day" in result[0]
        assert result[0]["total"] >= 1

    def test_stats_monthly_uses_month_column(self, tmp_db):
        """stats_monthly 使用 month 列查询。"""
        tmp_db.create_request("s2", "p2", "1:1", False)
        tmp_db.flush()
        result = tmp_db.stats_monthly(12)
        assert len(result) >= 1
        assert "month" in result[0]
        assert result[0]["total"] >= 1

    def test_stats_daily_old_data_still_works(self, tmp_db):
...
class TestCleanupAnalyze:
    ...
    def test_cleanup_runs_analyze(self, tmp_db):
        """cleanup 执行完毕不报错（含 ANALYZE）。"""
        result = tmp_db.cleanup(365)
