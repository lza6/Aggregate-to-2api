...
class TestRegistryDegrade:
    ...
    def test_degrade_marks_degraded(self):
        registry.degrade("aifreeforever", "test: 连续限流")
        assert registry.provider_health["aifreeforever"] == "degraded"

    def test_mark_down_marks_down(self):
        registry.mark_down("aifreeforever", "test: 不可用")
        assert registry.provider_health["aifreeforever"] == "down"
        # 同步更新 provider 实例
        p = registry.providers["aifreeforever"]
        assert p.health_status == "down"

    def test_recover_restores_health(self):
        registry.mark_down("aifreeforever", "test")
        registry.recover("aifreeforever")
        assert registry.provider_health["aifreeforever"] == "healthy"
        p = registry.providers["aifreeforever"]
        assert p.health_status == "healthy"

    def test_degraded_providers_ordering(self):
        """down 排在 degraded 前面。"""
        registry.degrade("imagefree", "test")
        registry.mark_down("aifreeforever", "test")
        degraded = registry.degraded_providers()
        assert degraded[0] == "aifreeforever"
        assert degraded[1] == "imagefree"

    def test_provider_for_returns_none_when_down(self):
        registry.mark_down("aifreeforever", "test")
        p = registry.provider_for("aifreeforever/gpt-image-2")
        # down 时如果有能力相同的备用 provider，返回备用 provider
        assert p is not None, "down 时应有备用 provider 接管"
        assert p.prefix != "aifreeforever", "应返回非 down 的备用 provider"
    ...
    def test_provider_for_returns_provider_when_healthy(self):
        ...
        assert p.prefix in ("aifreeforever", "aifreeforever", "imagefree")


# ── 连续 ProviderRateLimited 降级 ──────────────────
class TestConsecutiveFailures:
    def test_record_failure_counts(self):
        registry.record_failure("aifreeforever")
        assert registry._consecutive_failures["aifreeforever"] == 1
        registry.record_failure("aifreeforever")
        assert registry._consecutive_failures["aifreeforever"] == 2
    ...
    def test_record_success_resets_failure_count(self):
        registry.record_failure("aifreeforever")
        registry.record_failure("aifreeforever")
        registry.record_success("aifreeforever")
        assert registry._consecutive_failures.get("aifreeforever") is None
    ...
    def test_exhausted_accounts_tracking(self):
        registry.mark_exhausted("aifreeforever", "acc1@test.com")
        registry.mark_exhausted("aifreeforever", "acc2@test.com")
        assert registry._exhausted_accounts["aifreeforever"] == {"acc1@test.com", "acc2@test.com"}
...
class TestRecover:
    def test_try_recover_restores_health(self):
        registry.degrade("aifreeforever", "test")
        assert registry.provider_health["aifreeforever"] == "degraded"
        registry.try_recover("aifreeforever")
        assert registry.provider_health["aifreeforever"] == "healthy"
        assert registry._consecutive_failures.get("aifreeforever") is None
    ...
    def test_try_recover_all_skips_healthy(self, monkeypatch):
        ...
        registry._last_recover_at["aifreeforever"] = 0
        degraded = registry.degraded_providers()
        assert "aifreeforever" in degraded
        registry.try_recover_all()
        assert registry.provider_health["aifreeforever"] == "healthy"
...
class TestRouteIntegration:
    def test_provider_for_returns_429_when_down(self):
        ...
        registry.mark_down("aifreeforever", "test: 全量降级")
        provider = registry.provider_for("aifreeforever/gpt-image-2")
        # IMP-22 自动故障转移：down 时返回能力相同的备用 provider
        assert provider is not None, "应返回备用 provider 接管"
        assert provider.prefix != "aifreeforever", "备用 provider 不应是原 provider"
