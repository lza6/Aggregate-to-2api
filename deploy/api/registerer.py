...
class Minimaxh3Registerer:
    ...
    async def register_one(self) -> dict | None:
        ...
        # 1) 求解 turnstile（captchaToken；用 minimaxh3 自己的 sitekey + 注册页 /zh，代理=同一 kookeey IP）
        try:
...
class NanobananaRegisterer:
    ...
    def _ensure_client(self, email: str = "") -> None:
        """注册代理变化时重建 client；proxy 优先级：显式注入 > kookeey(email) > config.PROXY。"""
        want = self.proxy or kookeey_proxy_for(email) or config.PROXY
        if self._current_proxy != want:
            try:
                self.client.close()
    ...
    async def register_one(self) -> dict | None:
        ...
        # 1) turnstile + sign-up（用 nanobanana 自己的 sitekey + 注册页 /zh，代理=同一 kookeey IP）
        try:
            captcha, _ = await turnstile_client.solve_turnstile(
                config.CF_SOLVER_URL, self.turnstile_page, self.SITEKEY, config.TURNSTILE_TIMEOUT,
                proxy=kk)
        ...
        # 立即丢弃并标记 error（而非重试撞同邮箱），让补号循环尽快换下一邮箱，避免长期空转。
        ...
        email_pool.record(email, self.provider, "ok",
                          note="no_verify" if not link else "verified")
