...
// Providers 页排序逻辑 —— 不需要账号的提供商排前，需要账号的排末尾（用 aifreeforever 占位）。
...
describe('Providers 页 v6.9.1 排序逻辑', () => {
  it('needs_account=False（imagefree/aifreeforever）排到「无需账号」组', () => {
    const entries: ProviderEntry[] = [
      { prefix: 'aifreeforever', summary: base({ needs_account: true }) },
    ...
    expect(withAccount.map(e => e.prefix)).toEqual(['aifreeforever']);
  });
