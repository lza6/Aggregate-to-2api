"""号池管理器 AccountPool + 单例（P0-1 从 account_pool.py 拆出）。

`AccountPool` 是单一巨型类，方法共享 `self._conn`/`self._lock`/`self._db_path`，
因此整体搬迁到本模块，内部逻辑不动（仅 import 路径调整：FSM/scoring 改从包内导入）。
向后兼容：`api.account_pool.AccountPool` / `api.account_pool.account_pool` 旧 import 路径仍可用。
"""
...
from ..providers.base import MOCK_REGISTER  # noqa: F401  (包顶层 re-export 源)
...
def _pkg_attr(name: str, default):
    ...
    拆分后定义物理位于 pool.py，`__init__.py` 的 re-export 是值拷贝——若 pool.py 仍读
    自身全局，patch 包命名空间不会命中（原版天然命中）。此处运行时解析包属性，保持旧契约。
    """
    import sys
            ...
            for r in rows:
                ...
                if ok:
                    ...
                    if retry >= max_retry:
                        ...
                        await conn.execute(
                            "UPDATE accounts SET cooling_since=?, note=?, updated_at=? "
                            "WHERE provider=? AND email=?",
