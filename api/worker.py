...
class Engine:
    ...
    async def start(self) -> None:
        ...
        log.info("引擎启动: workers=%d token_pool=%d 队列上限=%d auto_scale=%s",
    ...
    async def submit_priority(self, prompt: str, aspect_ratio: str, download: bool,
        ...
        try:
            if self._queue_counts[priority] >= limits[priority]:
    ...
    def _create_worker(self, idx: int) -> _WorkerHandle:
        """创建唯一 worker 句柄：启动 _worker_loop 协程。"""
    ...
    async def _worker_loop(self, idx: int, stop_event: asyncio.Event | None = None) -> None:
        while True:
            ...
            try:
                ...
                self.db.mark_finished(task_id, "error", None,
    ...
    async def _process(self, task_id: str) -> None:
        ...
        if config.IF_DLQ_ENABLED:
            ...
            self.db.push_dlq(task_id, model, last_error, config.IF_TXT_RETRY_MAX)
