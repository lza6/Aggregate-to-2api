...
async def main():
    ...
    reg_r = await reg_client.post("https://nanobanana-pro.com/api/auth/sign-up/email",
    ...
    for i in range(25):
        ...
        if mr.status_code == 200:
            ...
            if msgs and isinstance(msgs, list) and len(msgs) > 0:
                ...
                det_r = await c.get(f"https://web2.temp-mail.org/messages/{mid}", headers={"Authorization": f"Bearer {token}"})
    ...
    login_r = await reg_client.post("https://nanobanana-pro.com/api/auth/sign-in/email",
    ...
    if "__Secure-better-auth.session_token" in login_r.cookies:
        ...
        print("7. SUCCESS! Successfully added account to database! Total accounts in DB:", len(account_pool.get("nanobanana")))
    else:
