...
async def main():
    c = httpx.AsyncClient(headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Safari/537.36"})

    # 1. 建箱
    print("1. Creating temp-mail.io mailbox...")
    cr = await c.post("https://api.internal.temp-mail.io/api/v3/email/new", json={"min_name_length": 10, "max_name_length": 10})
    ...
    reg_r = await c.post("https://nanobanana-pro.com/api/auth/sign-up/email",
    ...
    login_r = await c.post("https://nanobanana-pro.com/api/auth/sign-in/email",
